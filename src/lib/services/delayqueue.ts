...
export class Service<A> {
  ...
  public key(name: keyof A, id: string) {
    const { ns } = this.args;
    return [ns, name, id].join(KEY_SEP);
  }
  ...
  private async _runInterval() {
    ...
    this.timer = setTimeout(() => {
      this._runInterval();
    }, pollInterval * 1000 + (Math.random() - 0.5) * 20);
    const { redis } = this;
    ...
    for (let i = 0; i < queueValues.length; i = i + 2) {
      ...
      const task = async () => {
        ...
        const task = await redis.eval(getTaskScript, 5, this.idsKey, this.pendingIdsKey, key, this.lockedKey(key), this.pendingKey(key), this.args.pollInterval * 1000 + 100);
        if (task) {
