...
if (!jsonaFile || !prefix || !format) {
  console.log(
    "Usage: node ./list-routes.js <jsona-file> <prefix> <simple|json|yaml> [operationSuffix]"
  );
  process.exit();
}
...
if (format === "json") {
  const result = operations.reduce((acc, cur) => {
    Object.assign(acc, {
      [`${cur.operationId}${operationSuffix}`]: {
        req: { url: withPrefix(cur.path), method: cur.method },
      },
    });
    return acc;
  }, {});
  ...
  operations.forEach((o) => {
    console.log(
      `${
        o.operationId
      }${operationSuffix}: "${o.method.toUpperCase()} ${withPrefix(o.path)}",`
    );
  });
}
